...
public class ManageTask {
  ...
  //1. add task
    public int addTask(String requirementName, String taskType, String date, double from,
            double to, String assignee, String reviewer) {

        //check duplicate => duplicate => return -1 
        if (!checkDup(requirementName, taskType, date, from, to, assignee, reviewer)) {
            return -1;
        } //else return ids
        else {
            //create bien instance
            Task task = new Task(requirementName, taskType, date, from, to, assignee, reviewer);
            //add to list
            listTask.add(task);
            //return
            return task.getId();
    ...
    //check dup
    public boolean checkDup(String requirementName, String taskType, String date, double from,
            double to, String assignee, String reviewer) {
        for (Task task : listTask) {
            if (task.getName().equals(requirementName)
